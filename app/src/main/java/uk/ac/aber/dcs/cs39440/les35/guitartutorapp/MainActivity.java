...
public class MainActivity extends AppCompatActivity {
    ...
    public void processPitch(float pitchInHz) throws IOException {
        ...
        if (pitchInHz >= currentClosestNote.getFrequency() - (5 * centDown) && pitchInHz <= currentClosestNote.getFrequency() + (5 * centUp)) {
    ...
    private void setGaugeRotation(float pitchInHz, Note currentClosestNote, float centDown, float centUp, Note previousNote, Note nextNote) {
        if (pitchInHz < currentClosestNote.getFrequency()) {
            ...
            float angle;
            for (int i = 1; i < 100; i++) {
                if (pitchInHz > currentClosestNote.getFrequency() + (i * centDown) && pitchInHz < nextNote.getFrequency() - ((99 - i) * centDown)) {
                    needle.clearAnimation();
    ...
    private void setupSpinners() {
        ArrayAdapter<CharSequence> instrumentAdapter = ArrayAdapter.createFromResource(this, R.array.instrument_spinner_array, android.R.layout.simple_spinner_item);
